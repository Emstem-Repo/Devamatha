...
//Detail Mark Entry JS
 ...
 function submitEditCancelButton(){
	 	document.location.href="applicationEdit.do?method=forwardConfirmPage";
	 }
//All resets...
 ...
 function clearField(field){
	...
	function calctotalMarkWithoutLan(count)
	 ...
	 function getCourseForOnline(program,destId) {
			destID=destId;
			getCoursesByProgramForOnline("coursesMap",program,destId,updateCourse);	
		}
	 
	 
	 //rank
	 function getSubCaste(religion,destId) {
		 if(document.getElementById(destId)!=null){
				destID=destId;
		getSubCasteByReligion("subCasteMap",religion,destId,updateSubCaste);
		 }
	}

	function updateSubCaste(req) {
		
		var relId=document.getElementById("religions").value;
		if(relId==3)
		{
			updateOptionsFromMap(req,destID,"-Select-");
		}
	...
	function submitApplicationEditForm(method){
	...
	function getPermAddrDistrict(country,destId) { 
				destID=destId;
				getDistrictByState("permAddrDistrictMap",country,destId,updateDistrict);	
	}
	
	function getTempAddrDistrict(country,destId) { 
				destID=destId;
				getDistrictByState("tempAddrDistrictMap",country,destId,updateDistrict);	
	}
