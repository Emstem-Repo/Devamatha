...
public class Core4 {
	...
	/*if (attendanceEntryForm.getBatchId() != null && attendanceEntryForm.getBatchId().length() != 0) {
		int batchId = Integer.parseInt(attendanceEntryForm.getBatchId());
		 batchStudentList = attendanceEntryTransaction.getStudentByBatch(batchId);
		...
		for (Integer integer : attendanceEntryForm.getBatchList()) {
			List<BatchStudent> batchStudents = attendanceEntryTransaction.getStudentByBatch(integer);
		...
		for (String batchId : attendanceEntryForm.getBatchIdsArray()) {
			...
			List<BatchStudent> batchStudents = attendanceEntryTransaction.getStudentByBatch(Integer.parseInt(batchId));
