...
import com.kp.cms.handlers.teacherfeedback.EvaTeacherFeedbackOpenConnectionHandler;
import com.kp.cms.to.teacherfeedback.EvaTeacherFeedbackOpenConnectionTo;
import com.kp.cms.utilities.CommonUtil;
import com.kp.cms.utilities.CurrentAcademicYear;

public class EvaTeacherFeedbackOpenConnectionAction extends BaseDispatchAction {
	private static final Log log = LogFactory
	.getLog(EvaTeacherFeedbackOpenConnectionAction.class);
...
* @throws Exception
*/
public ActionForward initTeacherFeedbackOpenConnection(
	ActionMapping mapping, ActionForm form, HttpServletRequest request,
	HttpServletResponse response) throws Exception {
EvaTeacherFeedbackOpenConnectionForm connectionForm = (EvaTeacherFeedbackOpenConnectionForm) form;
...
try {
	...
	log .error("Error while initializing initTeacherFeedbackOpenConnection" + e.getMessage());
	String msg = super.handleApplicationException(e);
	connectionForm.setErrorMessage(msg);
	connectionForm.setErrorStack(e.getMessage());
	return mapping.findForward(CMSConstants.ERROR_PAGE);
}
return mapping .findForward(CMSConstants.TEACHER_FEEDBACK_OPEN_CONNECTION);
}

private void setSessionList( EvaTeacherFeedbackOpenConnectionForm connectionForm, HttpServletRequest request) throws Exception{
Map<Integer,String> feedbackSessionMap = EvaTeacherFeedbackOpenConnectionHandler.getInstance().getSessionList(connectionForm);
...
* @throws Exception
*/
private void setOpenConnectionList( EvaTeacherFeedbackOpenConnectionForm connectionForm) throws Exception {
...
//end
List<EvaTeacherFeedbackOpenConnectionTo> toList = EvaTeacherFeedbackOpenConnectionHandler.getInstance().getDetails(year,sessionId);
...
private void setClassesToRequest(
	EvaTeacherFeedbackOpenConnectionForm connectionForm,
...
try {
	if (errors.isEmpty()) {
		...
		setUserId(request, connectionForm);
		boolean duplicatecheck = EvaTeacherFeedbackOpenConnectionHandler
				.getInstance().duplicateCheck(connectionForm);
		if (!duplicatecheck) {
			boolean isAdded = EvaTeacherFeedbackOpenConnectionHandler
...
try {
	setUserId(request, connectionForm);
	EvaTeacherFeedbackOpenConnectionHandler.getInstance()
...
setSpecializationListByClass(connectionForm, request);
return mapping
		.findForward(CMSConstants.TEACHER_FEEDBACK_OPEN_CONNECTION);
}

private void setSpecializationListByClass( EvaTeacherFeedbackOpenConnectionForm connectionForm, HttpServletRequest request)throws Exception {
...
try {
	if (errors.isEmpty()) {
		boolean isDeleted = EvaTeacherFeedbackOpenConnectionHandler
...
try {
	if (errors.isEmpty()) {
		boolean duplicatecheck = EvaTeacherFeedbackOpenConnectionHandler
				.getInstance().duplicateCheck(connectionForm);
		if (!duplicatecheck) {
			boolean isUpdated = EvaTeacherFeedbackOpenConnectionHandler
